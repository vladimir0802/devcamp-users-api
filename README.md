...
Pazi: `prisma.user.findUnique()` **ne baca grešku** kad korisnik ne postoji —
vrati `null`. Zato 404 moraš vratiti sam:

```ts
if (!user) return res.status(404).json({ error: "User not found" });
```

Kod `delete` je obrnuto — Prisma baca grešku, pa tamo ide `try/catch`.
